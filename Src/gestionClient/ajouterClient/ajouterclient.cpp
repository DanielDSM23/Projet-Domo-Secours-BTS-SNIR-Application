...
void ajouterClient::on_ajouter_clicked()
{
    ...
    if(bddIHM->getDb().isOpen() && (canBeSended[0] & canBeSended[1])){
        ...
        if(poidsClient == "''"){poidsClient = "NULL";}
        if(tailleClient == "''"){tailleClient = "NULL";}
        if(numeroSecoursClient == "''"){numeroSecoursClient = "NULL";}
        if(numeroMedecinClient== "''"){numeroMedecinClient = "NULL";}
        if(pathologiesClient== "''"){pathologiesClient = "NULL";}
        query.exec("INSERT INTO `med_client` (`info_client_client_id`, `sexe`, `dateNaissance`, `taille`, `poids`, `numMedecin`, `numECU`, `infoMedPAT`)"
                   "VALUES ('"+clientID+"', '"+sexe+"', '"+dateNaissanceClient+"', "+tailleClient+", "+poidsClient+", "+numeroMedecinClient+", '"+numeroEcuClient+"', "+pathologiesClient+");");
        if(sexe == "F"){
        ...
        QString argumentsStr = "-f domosecours@alwaysdata.net -t "+mailClient+" -smtp smtp-domosecours.alwaysdata.net -sub \"Bienvenue sur Domo-Secours\" -ssl -port 465 -auth -user dmsc@alwaysdata.net -pass q9Z99Xvh -cs \"utf-8\" -mime-type \"text/plain\" -msg-body \"message.txt\"";
